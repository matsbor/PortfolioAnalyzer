...
def calculate_alpha_models(row, hist_data, benchmark_data):
    ...
    # MODEL 7: SMC (Smart Money Concepts)
    smc_score = row.get('SMC_Score', 50)
    models['M7_SMC'] = {
        'score': smc_score,
        'weight': 0.08,
        'description': f"SMC {row.get('SMC_Bias', 'Neutral')}: {smc_score:.0f}/100"
    }
...
def calculate_smc_signals(hist_data, current_price):
    ...
    Identifies: BOS, CHOCH, liquidity sweeps, order blocks
    """
    result = {
        'bias': 'Neutral',
        'signals': [],
    ...
    try:
        ...
        # Find swing highs/lows (5-bar pivots)
        window = 5
        swing_highs_idx = []
        swing_lows_idx = []
        ...
        recent_highs = [df['High'].iloc[i] for i in swing_highs_idx[-3:]]
        recent_lows = [df['Low'].iloc[i] for i in swing_lows_idx[-3:]]
        ...
        if last_close > last_swing_high * 1.001 and bullish_structure:
            result['signals'].append('BOS↑')
            result['score'] += 15
        elif last_close < last_swing_low * 0.999 and bearish_structure:
            result['signals'].append('BOS↓')
            result['score'] -= 15
        
        # CHOCH (Change of Character)
        if last_close > last_swing_high * 1.001 and not bullish_structure:
            result['signals'].append('CHOCH↑')
            result['score'] += 12
        elif last_close < last_swing_low * 0.999 and not bearish_structure:
            result['signals'].append('CHOCH↓')
            result['score'] -= 12
        
        # Liquidity sweeps
        for bar in df.tail(10).itertuples():
            if bar.High > last_swing_high and bar.Close < last_swing_high:
                if 'SweepHigh' not in result['signals']:
                    result['signals'].append('SweepHigh')
                    result['score'] -= 5
            if bar.Low < last_swing_low and bar.Close > last_swing_low:
                if 'SweepLow' not in result['signals']:
                    result['signals'].append('SweepLow')
                    result['score'] += 8
        
        # Order blocks
        if any(s in result['signals'] for s in ['BOS↑', 'CHOCH↑']):
            red_candles = df[df['Close'] < df['Open']].tail(10)
            if not red_candles.empty:
                ob_level = red_candles.iloc[-1]['Close']
                if abs(current_price - ob_level) / current_price < 0.05:
                    result['signals'].append('NearBullOB')
                    result['score'] += 5
        ...
        if result['score'] >= 65:
            result['bias'] = 'Bullish'
            result['summary'] = f"Bullish: {', '.join(result['signals'][:3])}"
        elif result['score'] <= 35:
            result['bias'] = 'Bearish'
            result['summary'] = f"Bearish: {', '.join(result['signals'][:3])}"
        else:
            result['bias'] = 'Neutral'
            result['summary'] = f"Mixed: {', '.join(result['signals'][:2]) if result['signals'] else 'Range'}"
    
    except:
        result['summary'] = 'SMC calc error'
...
# ============================================================================
# ENHANCED NEWS WITH TIMESTAMP FIX
# ============================================================================

def normalize_timestamp(ts):
    """Fix timestamp issues (no more 1964/1970)"""
    if not ts:
        return None
    try:
        if ts > 1e12:
...
def get_sector_news_fallback():
    ...
    try:
        all_news = []
        for ticker in ['GDXJ', 'SILJ', 'GC=F', 'SI=F']:
            try:
                stock = yf.Ticker(ticker)
                for item in stock.news[:2]:
                    ts = normalize_timestamp(item.get('providerPublishTime'))
                    all_news.append({
                        'title': f"[{ticker}] {item.get('title', '')}",
                        'publisher': item.get('publisher', 'Market'),
                        'date_str': datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d') if ts else 'Recent'
                    })
            except:
                pass
        return all_news[:8]
...
def get_news_for_ticker(ticker):
    """Fetch and tag news with fixed timestamps"""
    if not YFINANCE:
        return []
    
    try:
        stock = yf.Ticker(ticker)
        news = stock.news[:25]  # Fetch up to 25 items
...
if st.button("🚀 RUN WORLD-CLASS ANALYSIS", type="primary", use_container_width=True):
    ...
    progress.progress(78, text="📈 Calculating SMC signals...")
    ...
    for idx, row in df.iterrows():
        ...
        df.at[idx, 'SMC_Bias'] = smc['bias']
        df.at[idx, 'SMC_Score'] = smc['score']
        df.at[idx, 'SMC_Summary'] = smc['summary']
        
        smc_signals_storage[row['Symbol']] = smc['signals']
    ...
    for idx, row in df.iterrows():
        ...
        # INSTITUTIONAL ENHANCEMENT: Strict discovery exception
...
def add_ranking_columns(df):
    """Add ranking columns for sorting"""
    ACTION_RANK = {
        '🟢 STRONG BUY': 7, '🟢 BUY': 6, '🔵 ADD': 5, '🔵 ACCUMULATE': 5,
...
def render_daily_summary(df, macro_regime, cash):
    """Render executive summary at top of page"""
    ...
    for action in ['🟢 STRONG BUY', '🟢 BUY', '🔵 ADD', '⚪ HOLD']:
        if action_counts.get(action, 0) > 0:
            col4.caption(f"{action}: {action_counts[action]}")
